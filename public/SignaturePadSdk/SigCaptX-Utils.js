...
function print(txt) {
  ...
  if (txtDisplay) {
      ...
      console.error("Element with ID 'txtDisplay' not found.");
  }
}
// function print(txt) 
// {
...
//print clearTextBox callbackStatusOK
...
function showSignature()
{
  ...
  function onGetSignature(sigCtlV, sigObjV, status)
  {
    if(wgssSignatureSDK.ResponseStatus.OK == status)
    {
      ...
      if (imageBox) {
        var width = imageBox.clientWidth;  // Only access if the element exists
        var height = imageBox.clientHeight;
        sigObj.RenderBitmap(BITMAP_IMAGEFORMAT, width, height, BITMAP_INKWIDTH, BITMAP_INKCOLOR, BITMAP_BACKGROUNDCOLOR, outputFlags, BITMAP_PADDING_X, BITMAP_PADDING_Y, onRenderBitmap);
      } else {
...
function addRectangle( rectangleObj, callbackRoutine)
{
  ...
  var x3Var = new wgssSignatureSDK.Variant();
  var y3Var = new wgssSignatureSDK.Variant();
  var objData = new wgssSignatureSDK.Variant();
  var options = new wgssSignatureSDK.Variant();
  ...
  x3Var.Set(rectangleObj.x3Pos);
  y3Var.Set(rectangleObj.y3Pos);
